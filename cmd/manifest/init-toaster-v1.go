...
// RunToasterInitV1 initializes a new okteto manifest based on Toaster
func (mc *Command) RunToasterInitV1(ctx context.Context, opts *InitOpts) error {
	//getting entity name by current folder name
	dimValue := filepath.Base(opts.Workdir)

	// Open menifest json from platform inventory folder
	piManifestByte, err := os.ReadFile(fmt.Sprintf("examples/inventory/demo-org/application/%s.json", dimValue))
	if err != nil {
		oktetoLog.Error(err)
		return err
	}

	oktetoLog.Success(fmt.Sprintf("Opened examples/inventory/demo-org/application/%s.json", dimValue))
	...
	for i := 0; i < len(piManifest.Services) && len(piManifest.Services) != 0; i++ {
		devs[piManifest.Services[i].Identity] = generateToasterDev(piManifest.Services[i].Identity, "service")
	...
	for i := 0; i < len(piManifest.Workers) && len(piManifest.Workers) != 0; i++ {
		devs[piManifest.Workers[i].Identity] = generateToasterDev(piManifest.Workers[i].Identity, "worker")
		//identitiesList = append(identitiesList, piManifest.Services[i].Identity)
	}

	for i := 0; i < len(piManifest.KConsumers) && len(piManifest.KConsumers) != 0; i++ {
		devs[piManifest.KConsumers[i].Identity] = generateToasterDev(piManifest.KConsumers[i].Identity, "kconsumer")
		//identitiesList = append(identitiesList, piManifest.Services[i].Identity)
	}

	for i := 0; i < len(piManifest.Schedulers) && len(piManifest.Schedulers) != 0; i++ {
		devs[piManifest.Schedulers[i].Identity] = generateToasterDev(piManifest.Schedulers[i].Identity, "scheduler")
		//identitiesList = append(identitiesList, piManifest.Services[i].Identity)
	}
...
func generateToasterDev(devName string, workloadType string) *model.Dev {
	dev := &model.Dev{
		Container:            devName,
		Workdir:              "/appdev",
